...
showTerm (Fn (Skolem v@(V s) n) []) = text "sK'" <> text s <> (if n == 1 then mempty else text ("." ++ show n)) <> text "'"
showTerm (Fn (Skolem v@(V s) n) args) = text "sK" <> text s <> (if n == 1 then mempty else text ("." ++ show n)) <> (brackets (hcat (punctuate (char ',') (map showTerm args))))
